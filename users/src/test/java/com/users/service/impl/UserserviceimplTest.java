...
class UserserviceimplTest {
    ...
    void updateUser() {
        ...
        userDto.setEmail("srijanshrestha@gmail.com");
    ...
    void findUserByTime() {
        ...
        Date end = new Date();
        when(userRepo.findByCreatedDateBetweenAndId(start, end,1L)).thenReturn(List.of(mockUser));
        when(modelMapper.map(any(Users.class), eq(UserResponseDto.class))).thenReturn(new UserResponseDto());

        List<UserResponseDto> result = userserviceimpl.findUserByTime(start, end,1L);
        assertEquals(1, result.size());
    }
    ...
    void changePassword() {
        ...
        String result = userserviceimpl.changePassword(email, dto);
